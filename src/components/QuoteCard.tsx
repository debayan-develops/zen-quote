...
export default function QuoteCard() {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(true); // Corrected variable name
  const [error, setError] = useState<string | null>(null);

  const fetchQuote = async () => {
    setIsLoading(true); // Corrected variable name
    setError(null);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg max-w-md w-full text-center transition-all duration-300 ease-in-out">
      <div className="min-h-[100px] flex items-center justify-center mb-6">
        {isLoading ? ( // Corrected variable name
          ...
          <blockquote className="text-xl font-medium text-gray-700 dark:text-gray-300">
            {/* FIX: Removed the extra literal double quotes */}
            {quote}
          </blockquote>
        )}
      </div>
      ...
      <button
        onClick={fetchQuote}
        disabled={isLoading} // Corrected variable name
        className={`
        ...
        `}
      >
        {isLoading ? 'Loading...' : 'Get New Quote'} {/* Corrected variable name */}
      </button>
    </div>
  );
}
